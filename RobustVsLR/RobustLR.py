...
validation_threshold = dimension // 2
...
for i in range(number_of_features):
    ...
    for j in range(i, number_of_features):
        ...
        if sample_size < 2:  # Change max to max_abs, and if we change the threshold, does it change the performance?
            max_vals = columns.abs().max()
...
for i in range(len(mask_X)):
    ...
    if not np.isnan(data[['critical_temp']].loc[i][0]):
        predicts.append(data[['critical_temp']].loc[i][0])
        predicts1.append(data[['critical_temp']].loc[i][0])
        predicts2.append(data[['critical_temp']].loc[i][0])
        continue
