...
# data = data[np.isfinite(data['280'])]
...
X = data[data.columns[0:280]]
X = X.drop(column_numbers, axis=1)

# X.to_csv("ForMice.csv", index=None)
# exit(0)
mask_X = X.isna()
...
X_test = test[test.columns[0:280]]
...
np.savetxt("conf_matrix_nmar50.csv", confidence_matrix, delimiter=",")
np.savetxt("conf_list_nmar50.csv", conf_list, delimiter=",")
# exit(0)
"""
confidence_matrix = np.loadtxt('conf_matrix_nmar50.csv', delimiter=',')
conf_list = np.loadtxt('conf_list_nmar50.csv', delimiter=',')
