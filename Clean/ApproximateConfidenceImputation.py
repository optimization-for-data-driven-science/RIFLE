...
data = pd.read_csv('missing_MCAR_P80_500.csv')

data_validation = data[np.isfinite(data['critical_temp'])]

data_validation = data_validation.fillna(data_validation.mean())
Y_validation = data_validation[['critical_temp']]
X_validation = data_validation.drop(['critical_temp'], axis=1)
...
lam_list = [0.1, 0.5, 1, 1.5, 2, 3, 4, 5, 6, 7, 8, 9, 10, 20]
...
original = pd.read_csv('original_MCAR_P80_500.csv')
