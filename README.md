...
*Figure 1: Consider the problem of predicting the trait y from feature vector (x<sub>1</sub>, \dots, x<sub>100</sub>). Suppose that we have access to three data sets: The first dataset includes the measurements of $(x<sub>1</sub>, x<sub>2</sub>, \dots ,x<sub>40</sub>, y)$ for $n<sub>1</sub>$ individuals. The second data set collects data from another $n<sub>2</sub>$ individuals by measuring $(x<sub>30</sub>, \dots, x<sub>80</sub>)$ with no measurements of the target variable $y$ in it; and the third data set contains the measurements from the variables $(x<sub>70</sub>, \dots, x<sub>100</sub>, y)$ for $n<sub>3</sub>$ number of individuals; How one should learn the predictor $\hat{y} = \delta(x<sub>1</sub>, \dots, x<sub>100</sub>)$ from these three datasets?*
