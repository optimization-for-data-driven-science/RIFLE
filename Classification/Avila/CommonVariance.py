...
train = pd.read_csv('train_missing_MCAR50_40_2.csv')
...
for iteration in range(number_of_iterations):
    ...
    for i in range(number_of_estimations):
        current_data = np.random.multivariate_normal(mu1s[i], sigmas[i], size=number_of_samples)

        res = - np.log(sigmoid(np.dot(current_data, w)))
        a1[i] = res.mean()

    for i in range(number_of_estimations):
        current_data = np.random.multivariate_normal(mu0s[i], sigmas[i], size=number_of_samples)
    ...
    while fabs(sum_of_probabilities - 1) > epsilon:
        ...
        if t > 1000:
            print("Stuck!")
            print("Sum: ", sum_of_probabilities)
            break
    ...
    for counter in range(number_of_estimations):
        data_batch = np.random.multivariate_normal(mu1s[counter], sigmas[counter], size=number_of_samples)
    ...
    for counter in range(number_of_estimations):
        data_batch = np.random.multivariate_normal(mu0s[counter], sigmas[counter], size=number_of_samples)
