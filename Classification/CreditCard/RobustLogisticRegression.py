...
X_1 = train.loc[train['default payment next month'] == 1.0]
X_0 = train.loc[train['default payment next month'] == 0.0]

X_1 = X_1.drop(['default payment next month'], axis=1)
columns = X_1.columns
ind = X_1.index
X_1 = sc.transform(X_1)
X_1 = pd.DataFrame(X_1, columns=columns, index=ind)

X_0 = X_0.drop(['default payment next month'], axis=1)
columns = X_0.columns
ind = X_0.index
...
for iteration in range(number_of_iterations):
    ...
    for i in range(number_of_estimations):
        current_data = np.random.multivariate_normal(mu_1s[i], sigma_1s[i], size=5000)

        res = - np.log(sigmoid(np.dot(current_data, w)))
        a1[i] = res.mean()

    for i in range(number_of_estimations):
        current_data = np.random.multivariate_normal(mu_0s[i], sigma_0s[i], size=5000)
    ...
    for counter in range(number_of_estimations):
        data_batch = np.random.multivariate_normal(mu_1s[counter], sigma_1s[counter], size=5000)
